...
OWNER_ID = ('1419969308','5792543632')  # Replace with your owner ID
...
def start(message):
    ...
    if user_id not in load_approved_users():
        bot.reply_to(message, "𝘠𝘰𝘶 𝘢𝘳𝘦 𝘯𝘰𝘵 𝘢𝘱𝘱𝘳𝘰𝘷𝘦𝘥 𝘵𝘰 𝘶𝘴𝘦 𝘵𝘩𝘪𝘴 𝘣𝘰𝘵. 𝘊𝘰𝘯𝘵𝘢𝘤𝘵 𝘵𝘩𝘦 𝘰𝘸𝘯𝘦𝘳- @RAHUL_RESPECT")
        return
...
def handle_document(message):
    ...
    if user_id in load_banned_users():
        ...
        return
    if user_id not in load_approved_users():
        bot.reply_to(message, "𝘊𝘰𝘯𝘵𝘢𝘤𝘵 𝘵𝘩𝘦 𝘰𝘸𝘯𝘦𝘳- @RAHUL_RESPECT")
        return
